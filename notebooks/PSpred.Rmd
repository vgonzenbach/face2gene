...
tmp_df = data.frame(group = as.factor(penn_df$group),
          PC1, PC2) %>% filter(group!= "NC") 
tmp_df %>% ggplot(aes(x=PC1, y=PC2, color = group)) + geom_point() + 
...
```{r, message=FALSE}
resROCPC2 = roc(df$group ~ PC2, data=NULL)
...
```{r}
cov_df = data.frame(PC2, penn_df[, c("group", "sex", "age", "race")])
```

### Sex

```{r}
anova(lm(PC2 ~ sex, data=cov_df))
```

### Race

```{r}
anova(lm(PC2 ~ race, data=cov_df))
```

### Age

```{r}
cor.test(cov_df$PC2, cov_df$age)
anova(lm(PC2 ~ age, data=cov_df))
```

## Age, Sex, Race

```{r}
anova(lm(PC2 ~ sex + race + age, data=cov_df))
```
