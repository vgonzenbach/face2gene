---
title: "Predicting PS status with 22q characteristics"
author: "Virgilio Gonzenbach"
date: "`r format(Sys.time(), '%d %B, %Y')`"
output: 
  html_document:
    toc: true
    toc_float: True
    theme: 'spacelab'
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = FALSE,
                      warning = FALSE,
                      message = FALSE,
                      cache = FALSE,
                      cache.path = 'cache/PSpred/', 
                      fig.path = "figures/PSpred/")
knitr::opts_knit$set(root.dir = rprojroot::find_rstudio_root_file())
```


```{r load, include=FALSE}
library(dplyr)
library(tidyr)
library(FactoMineR)
library(factoextra)
library(pander)
library(pROC)
library(kableExtra)

dat = read.csv('data/penn_chop_tp1_f2g_emotrics_passed_clean_030822.csv',
               check.names = FALSE) 
sz_cr = read.csv('data/penn_szcr_chop_tp1_wideV_f2g_030822.csv')
sz_cr = sz_cr[sz_cr$group %in% c('TD', 'SZ', 'CR'), c('bbl_id', 'group')]
chop_df = dat[dat$group == '22q', ]
penn_df = dat[dat$group != '22q', ]

# Exclude columns with only zeros
get_zero_cols = function(df){
  num_zeros = df %>% sapply(function(x) x == 0) %>% colSums()
  prop_zeros = num_zeros / nrow(df)
  zero_cols = names(which(num_zeros == nrow(df)))
  return(zero_cols)
}

chop_df = chop_df %>% select(-all_of(get_zero_cols(chop_df)))
penn_df = penn_df %>% select(-all_of(get_zero_cols(penn_df)))

# Set seed for reproducibility
seed=42
set.seed(seed)
```

In this analysis, psychosis spectrum (PS) status is predicted based on 22q11.2 deletion syndrome (22q) characteristics.  

- First, PCA is performed on the 22q sample to estimate the relative contributions, i.e. the loadings, of each gestalt score to the axes of greatest variability (PCs) in this group.

- These loadings are then used to compute "22q-like" factor scores for the TD and PS groups: ROC analysis shows **PC2 alone correctly predicts PS status 67.8% of the time**.  

- Additionally, Levene’s test shows that **variance from the schizophrenia (SZ) group differs from that of clinical risk (CR) on PC2**.

## 0. Sample sizes

Total sample sizes in 03/08/22 data (first row) and non-missing data per variable:  

```{r}
# Get total in extended data
vars= c("height", "weight", "GAF_C", "GAF_H", "MMSE", "VIQ", "PIQ", "FSIQ", "DSM Dx", "PS")  
n_df = data.frame(case_id = dat$case_id, 
                  #group = dat$group,
                  !is.na.data.frame(dat[,vars]),
                  check.names = FALSE)

n_df = merge(read.csv('data/penn_szcr_chop_tp1_wideV_f2g_030822.csv')[, c("case_id", "group")], n_df, by='case_id')
n_size = t(table(n_df$group))
n_df = n_df %>% select(-case_id) %>% group_by(group) %>% summarize_all(sum)
n_tbl = rbind(n_size, t(n_df[,-1]))
n_tbl = cbind(n_tbl, rowSums(n_tbl))
colnames(n_tbl) = c(t(n_df)[1,], "total") 
n_tbl = as.table(n_tbl)
n_tbl %>% kable(format="html") %>% kable_styling("striped", full_width = TRUE)
```

Note: Emotrics available for all records.

## 1. Examining characteristics of 22q sample with PCA

PCA was performed without scaling on the 22q sample alone.

### Scree Plot

```{r}
syndromes = colnames(chop_df)[which(colnames(chop_df) == '22q11.2 deletion syndrome'):ncol(chop_df)]
demographics = colnames(chop_df)[1:(which(colnames(chop_df) == 'Brow_Height_Right') - 1)]
chop_df = chop_df[, c(demographics, syndromes)]
resPCA = PCA(chop_df[, syndromes], 
             scale.unit = FALSE, 
             graph = FALSE, 
             ncp=length(chop_df[,syndromes]))

fviz_screeplot(resPCA)
```

Using the "elbow rule", subsequent analysis will focus on the first 4 PCs. 

### Loadings

Loadings for the first 2 PCs are plotted below.

```{r}
get_loadings = function(resPCA, dim=1){
  # Extract metrics from PCA object
  r = resPCA$var$coord[,dim] / sqrt(resPCA$eig[dim,1])
  
  # is equal to 
  return(r)
}

plot_loadings = function(resPCA, dim=1){
  # Plot loadings
  r = get_loadings(resPCA, dim)
  filter = r^2 > mean(r^2, na.rm = TRUE) # only include loadings with more than average contributions
  
  # Prep data
  df = data.frame(var=names(r), cor = r, row.names = NULL)[filter, ] %>% drop_na()
  df = df[order(df$cor, decreasing = TRUE),]
  df$var = factor(df$var, levels = rev(df$var)) #order before plotting
  
  # Plot
  title = sprintf("PC%s contributions", dim)
  p = df %>% ggplot(aes(x=var, y=cor)) + geom_col() + coord_flip() + 
    xlab("Gestalt Score") + ylab("Loading") + ggtitle(title) + 
    theme_bw() + theme(axis.text.y = element_text(size=10))
  return(p)
}
```

#### PC1

```{r, fig.width=12, fig.height=8}
plot_loadings(resPCA, dim=1)
```

#### PC2

```{r, fig.width=12, fig.height=8}
plot_loadings(resPCA, dim=2)
```

#### Scores and effects

```{r}
cov_df = data.frame(chop_df[, c("case_id", "sex", "age", "race")], 
                    PC1 = resPCA$ind$coord[, 1], 
                    PC2 = resPCA$ind$coord[, 2])
anova(lm(PC2 ~ sex + age + race, data=cov_df))
```

#### Sex

```{r}
cov_df %>% 
  ggplot(aes(x=PC1, y=PC2, color = sex)) + 
  geom_point() + 
  geom_point(data = cov_df %>% 
               select(PC1, PC2, sex) %>% 
               group_by(sex) %>%  
               summarize_all(mean), size = 4, shape = 17) + 
  scale_color_manual(values = c("red", "blue")) + 
  ggtitle("22q-like PC scores for 22q sample coded by sex")
```

#### Age

```{r, message=FALSE, warning=FALSE}
cov_df %>% 
  ggplot(aes(x=age, y=PC2)) + 
  geom_point() + 
  geom_smooth(method='lm')
```

#### Emotrics

PC2 scores were correlated with Brow Height (Left and Right), Marginal Reflex Distance 2 (Left and Right), and Philtrum measurements. 

```{r}
emotrics = c('Brow_Height_Right', 'Brow_Height_Left', 'Marginal_Reflex_Distance_1_Right', 'Marginal_Reflex_Distance_1_Left', 'Marginal_Reflex_Distance_2_Right', 'Marginal_Reflex_Distance_2_Left', 'Philtrum')
emotrics_df = dat %>% select(case_id, all_of(emotrics)) 

# Add emotrics to scores data.frame
emo_df = merge(cov_df, emotrics_df, by = 'case_id')
res.cor = psych::corr.test(emo_df[, c("PC1", "PC2")], 
                       emo_df[, emotrics], 
                       adjust='fdr')

cor.mat = rbind(res.cor$r[1,], 
                res.cor$p.adj[1,], 
                res.cor$r[2,], 
                res.cor$p.adj[2,])

rownames(cor.mat) = c('PC1', 'p', 'PC2', 'p')
cor.mat %>% t() %>% kable(format = 'html') %>%  kable_styling("striped", full_width = TRUE)
sig.emotrics = names(which(cor.mat[4,] < .05))
```

#### IQ

There were no significant relationships with IQ.

```{r}
iq = c("VIQ", "PIQ", "FSIQ")
iq_df = dat %>% select(case_id, all_of(iq)) 

# Add emotrics to scores data.frame
iq_df = merge(cov_df, iq_df, by = 'case_id')
res.cor = psych::corr.test(iq_df[, c("PC1", "PC2")], 
                       iq_df[, iq], 
                       adjust='fdr')

cor.mat = rbind(res.cor$r[1,], 
                res.cor$p.adj[1,], 
                res.cor$r[2,], 
                res.cor$p.adj[2,])

rownames(cor.mat) = c('PC1', 'p', 'PC2', 'p')
cor.mat %>% t() %>% kable(format = 'html') %>%  kable_styling("striped", full_width = TRUE)
sig.iq = names(which(cor.mat[4,] < .05))
```

#### Height

There were no significant relationships with height.

```{r}
height_df = dat %>% select(case_id, all_of("height")) 

# Add emotrics to scores data.frame
height_df$height = sapply(height_df$height,
                          FUN = function(x) {
                            x = unlist(strsplit(x, "'"))
                            ft = as.numeric(x[1]) * 12
                            inch = as.numeric(x[2])
                            ft + inch
                          })
height_df = merge(cov_df, height_df, by = 'case_id')
res.cor = psych::corr.test(height_df[, c("PC1", "PC2")], 
                           height_df[, "height"],
                           adjust='fdr')

cor.mat = rbind(res.cor$r[1,], 
                res.cor$p.adj[1,], 
                res.cor$r[2,], 
                res.cor$p.adj[2,])

rownames(cor.mat) = c('PC1', 'p', 'PC2', 'p')
cor.mat %>% t() %>% kable(format = 'html') %>%  kable_styling("striped", full_width = TRUE)
sig.height = names(which(cor.mat[4,] < .05))
```


#### Weight

There were no significant relationships with weight.

```{r}
weight_df = dat %>% select(case_id, all_of("weight")) 

# Add emotrics to scores data.frame
weight_df = merge(cov_df, weight_df, by = 'case_id')
res.cor = psych::corr.test(weight_df[, c("PC1", "PC2")], 
                           weight_df[, "weight"],
                           adjust='fdr')

cor.mat = rbind(res.cor$r[1,], 
                res.cor$p.adj[1,], 
                res.cor$r[2,], 
                res.cor$p.adj[2,])

rownames(cor.mat) = c('PC1', 'p', 'PC2', 'p')
cor.mat %>% t() %>% kable(format = 'html') %>%  kable_styling("striped", full_width = TRUE)
sig.weight = names(which(cor.mat[4,] < .05))
```


#### GAF

There were no significant relationships with GAF.

```{r}
gaf = c("GAF_C", "GAF_H")
gaf_df = dat %>% select(case_id, all_of(gaf)) 

# Add emotrics to scores data.frame
gaf_df = merge(cov_df, gaf_df, by = 'case_id')
res.cor = psych::corr.test(gaf_df[, c("PC1", "PC2")], 
                       gaf_df[, gaf], 
                       adjust='fdr')

cor.mat = rbind(res.cor$r[1,], 
                res.cor$p.adj[1,], 
                res.cor$r[2,], 
                res.cor$p.adj[2,])

rownames(cor.mat) = c('PC1', 'p', 'PC2', 'p')
cor.mat %>% t() %>% kable(format = 'html') %>%  kable_styling("striped", full_width = TRUE)
sig.gaf = names(which(cor.mat[4,] < .05))
```

#### MMSE

There were no significant relationships with MMSE.

```{r}
mmse_df = dat %>% select(case_id, all_of("MMSE")) 

# Add emotrics to scores data.frame
mmse_df = merge(cov_df, mmse_df, by = 'case_id')
res.cor = psych::corr.test(mmse_df[, c("PC1", "PC2")], 
                           mmse_df[, "MMSE"],
                           adjust='fdr')

cor.mat = rbind(res.cor$r[1,], 
                res.cor$p.adj[1,], 
                res.cor$r[2,], 
                res.cor$p.adj[2,])

rownames(cor.mat) = c('PC1', 'p', 'PC2', 'p')
cor.mat %>% t() %>% kable(format = 'html') %>%  kable_styling("striped", full_width = TRUE)
sig.mmse = names(which(cor.mat[4,] < .05))
```


#### Pathology

```{r}
patho_df = dat[, c("case_id", "DSM Dx", "PS")] 

# Make factors
patho_df$`DSM Dx` = as.factor(recode(patho_df$`DSM Dx`, `1` = "Yes", `0` = "No"))
patho_df$PS = as.factor(recode(patho_df$PS, `1` = "Yes", `0` = "No"))
patho_df = merge(patho_df, cov_df, by="case_id") %>% drop_na()

## Plot DMS Dx
patho_df %>% 
  ggplot(aes(x=PC1, y=PC2, color = `DSM Dx`)) + 
  geom_point() + 
  geom_point(data = patho_df %>% select(PC1, PC2, `DSM Dx`) %>% 
               group_by(`DSM Dx`) %>%  
               summarize_all(mean), 
             size = 4, shape = 17) + 
scale_color_manual(values = c("magenta", "gold")) + 
  ggtitle("22q-like PC scores for 22q sample coded by DSM Dx")
```

```{r}
report.t = function(res.t, x, y){
  #' Report t-test in apa format
  #'' TODO: to fully automate: print groups from data.name, bonus: add formatting/substutions of var.name as parameter
  #' Make this work for a single test (check input is list)

    cat(sprintf("\n - %s (*M* = %s, *SD* = %s) and %s (*M* = %s, *SD* = %s), %s", 
            x,
            round(mean(res.t$data$x), 2), round(sd(res.t$data$x), 2),
            y,
            round(mean(res.t$data$y), 2), round(sd(res.t$data$y), 2),
            apa::apa(res.t, print = FALSE)), 
        sep="\n")
}
```



```{r}
## Plot PS
tmp_df = patho_df %>% select(PC1, PC2, PS) %>% drop_na()
tmp_df %>% 
  ggplot(aes(x=PC1, y=PC2, color = PS)) + 
  geom_point() + 
  geom_point(data = tmp_df %>% 
               select(PC1, PC2, PS) %>% 
               group_by(PS) %>%  
               summarize_all(mean), 
             size = 4, shape = 17) + 
  scale_color_manual(values = c("magenta", "gold")) + 
  ggtitle("22q-like PC scores for 22q sample coded by PS")
```

```{r, results='asis'}
x = patho_df$PC2[patho_df$PS == "Yes"]
y = patho_df$PC2[patho_df$PS == "No"]
report.t(apa::t_test(x, y), "PS=Yes", "PS=No")
```


## 2. Testing for differences in 22q-like factor scores between TD and PS

For the Penn sample (i.e. PS and TD groups), 2 factor scores are computed using the loadings above as weights in a linear combination of relevant gestalt scores (i.e. those gave hits in the 22q sample).

Note: any reference to PCs/factor scores below refers to those computed for the Penn sample. 

```{r}
weigh_scores = function(df, coefs){
  #' Returns a linear combination of named scores included in 'coefs' 
  #' If named scores in coefs are not defined in df, columns are initialized with 0
  
  # which gestalt scores are missing from penn_df? i.e. PS and TD groups
  nohits = names(coefs)[which(!(names(coefs) %in% colnames(df)))] 
  df[, nohits] = 0 # set missing to zero to define columns
  
  # linear combination of gestalt scores
  scores.mat = as.matrix(df[, names(coefs)])
  weighted_score = as.vector(scores.mat %*% coefs)
  return(weighted_score)
}

weigh_scores2 = function(df, resPCA){
  # which gestalt scores are missing from penn_df? i.e. PS and TD groups
  nohits = rownames(resPCA$var$coord)[which(!(rownames(resPCA$var$coord) %in% colnames(df)))] 
  df[, nohits] = 0 # set missing to zero to define columns
   
  scores.mat = as.matrix(df[, rownames(resPCA$var$coord)]) 
  weighted_score = scores.mat %*% resPCA$svd$V
  
  return(weighted_score)
}
```

### PC 1

No significant differences between groups.

```{r}
scores_df = data.frame(group = penn_df$group)
scores_df$PC1 = weigh_scores(penn_df[, which(colnames(penn_df) == '22q11.2 deletion syndrome'):ncol(penn_df)],
                             get_loadings(resPCA, 1))
# TD = df$group == "TD"
TD = penn_df$group == 'TD'
PS = penn_df$group == "PS"

res.t.test = t.test(scores_df$PC1[TD], scores_df$PC1[PS])
names(res.t.test$estimate) = c("mean of TD", "mean of PS")
res.t.test
```

### PC 2

PC2 shows a difference between TD and PS groups.

```{r}
scores_df$PC2 = weigh_scores(penn_df[, which(colnames(penn_df) == '22q11.2 deletion syndrome'):ncol(penn_df)],
                             get_loadings(resPCA, 2))

res.t.test = t.test(scores_df$PC2[TD], scores_df$PC2[PS])
names(res.t.test$estimate) = c("mean of TD", "mean of PS")
res.t.test
```

```{r}
scores_df %>% 
  ggplot(aes(x=PC1, y=PC2, color = group)) + 
  geom_point() + 
  geom_point(data = scores_df %>% group_by(group) %>% 
               summarize_all(mean), 
             size = 4, shape = 17) + 
  scale_color_manual(values = c("salmon", "dodgerblue")) + 
  ggtitle("22q-like PC Scores of Penn sample")
```


### SZ vs CR

We see a difference in variances on PC2.

```{r}
ps_td_df = merge(penn_df %>% select(!group), 
              sz_cr, by='bbl_id')
ps_td_df = data.frame(group = as.factor(ps_td_df$group),
                   PC1 = scores_df$PC1, 
                   PC2 = scores_df$PC2)

ps_df =  ps_td_df %>% 
  filter(group != "TD") 

ps_df %>% 
  ggplot(aes(x=PC1, y=PC2, color = group)) + 
  geom_point() + 
  geom_point(data = ps_df %>%  
               group_by(group) %>% 
               summarize_all(mean), 
             size = 4, shape = 17) + 
  scale_color_manual(values = c("orange", "purple"))
```

#### PC2: T-test

```{r}
SZ = ps_td_df$group=="SZ"
CR = ps_td_df$group=="CR"

res.t.test = t.test(scores_df$PC2[SZ], scores_df$PC2[CR])
names(res.t.test$estimate) = c("mean of SZ", "mean of CR")
res.t.test
```

#### PC2: Levene's test

```{r}
car::leveneTest(ps_df$PC2, ps_df$group)
```

#### Emotrics

```{r}
# Order emotrics 
emo_df = merge(data.frame(case_id = penn_df$case_id, 
                          bbl_id = penn_df$bbl_id,
                          scores_df), 
               emotrics_df, 
               by='case_id')
# emo_df %>%  select(all_of(emotrics)) %>% lapply(bartlett.test, g=emo_df$group) # All variances are equal after adj
res.t = emo_df %>% 
  select(all_of(emotrics)) %>% 
  lapply(function(x) apa::t_test(x[emo_df$group == 'PS'], x[emo_df$group == 'TD'], 
                                 var.equal=TRUE))

report.t.list = function(res.t){
  #' Report t-test in apa format
  #'' TODO: to fully automate: print groups from data.name, bonus: add formatting/substutions of var.name as parameter
    for(dep.var in names(res.t)){
      
        cat(sprintf("- %s: PS (*M* = %s, *SD* = %s) and TD (*M* = %s, *SD* = %s), %s", 
                gsub("Right", "(R)", gsub("Left", "(L)", gsub("_", " ", dep.var))),
                round(mean(res.t[[dep.var]]$data$x), 2), round(sd(res.t[[dep.var]]$data$x), 2),
                round(mean(res.t[[dep.var]]$data$y), 2), round(sd(res.t[[dep.var]]$data$y), 2),
                apa::apa(res.t[[dep.var]], print = FALSE)), 
            sep="\n")
    }
}
```

For univariate t-tests, the only difference between groups was in Philtrum measurements:  

```{r, results='asis'}
report.t.list(res.t)
```

#### GAF_C

GAF_C **are** significantly different between PS and TD.

```{r, results='asis'}
ps_td_gafc = dat %>% 
  select(group, GAF_C) %>% filter(group !='22q')

apa::t_test(ps_td_gafc$GAF_C[ps_td_gafc$group == 'TD'],
            ps_td_gafc$GAF_C[ps_td_gafc$group == 'PS'], var.equal=TRUE) %>% report.t('TD', 'PS')
```


#### GAF_H

GAF_H **are** significantly different between PS and TD.

```{r, results='asis'}
ps_td_gafh = dat %>% 
  select(group, GAF_H)

apa::t_test(ps_td_gafh$GAF_H[ps_td_gafh$group == 'TD'],
       ps_td_gafh$GAF_H[ps_td_gafh$group == 'PS'], var.equal=TRUE) %>% report.t('TD', 'PS')
```

#### Height

No significant difference.

```{r, results='asis'}
ps_td_height = dat %>% 
  select(group, height)

ps_td_height$height = sapply(ps_td_height$height,
                          FUN = function(x) {
                            x = unlist(strsplit(x, "'"))
                            ft = as.numeric(x[1]) * 12
                            inch = as.numeric(x[2])
                            ft + inch
                          })
apa::t_test(ps_td_height$height[ps_td_height$group == 'TD'],
       ps_td_height$height[ps_td_height$group == 'PS'], var.equal=TRUE) %>% report.t(x='TD', y='PS')
```

#### Weight

No significant difference.

```{r, results='asis'}
ps_td_weight = dat %>% 
  select(group, weight)

apa::t_test(ps_td_weight$weight[ps_td_weight$group == 'TD'],
            ps_td_weight$weight[ps_td_weight$group == 'PS'], var.equal=TRUE) %>% report.t(x='TD', y='PS')
```


#### MMSE

No significant difference.

```{r, results='asis'}
ps_td_mmse = dat %>% 
  select(group, MMSE)

apa::t_test(ps_td_mmse$MMSE[ps_td_mmse$group == 'TD'],
       ps_td_mmse$MMSE[ps_td_mmse$group == 'PS'], var.equal=TRUE) %>% report.t('TD', 'PS')
```

## 3. Predicting PS status based on 22q-like PC2

```{r}
plotROCs = function(roc1, roc2, roc3, main, legends){
  plot(roc1, col="black", main=main)
  plot(roc2, col="purple", add=TRUE)
  plot(roc3, col="orange", add=TRUE)
  
  AUC = c(round(as.vector(roc1$auc), 2),
          round(as.vector(roc2$auc), 2),
          round(as.vector(roc3$auc), 2))
  
  
  legend("bottomright", 
    legend = paste0(legends, " (", AUC, ")"),
    col = c("black", "purple", "orange"), 
    pch = c(17,19, 19), 
    pt.cex = 1, 
    cex = 1, 
    text.col = "black", 
    inset = c(0.1, 0.1))
}
```

```{r, message=FALSE, results='asis'}
resROCPC2_ps = roc(group ~ PC2, data=scores_df)

sz_df = data.frame(group = as.character(ps_td_df$group), 
                   PC2 = ps_td_df$PC2) %>% filter(group != 'CR')

resROCPC2_sz = roc(group ~ PC2, data=sz_df)

cr_df = data.frame(group = as.character(ps_td_df$group), 
                   PC2 = ps_td_df$PC2) %>% 
  filter(group != 'SZ')

resROCPC2_cr = roc(group ~ PC2, data=cr_df)

legends = c('PS', 'SZ', 'CR')
plotROCs(resROCPC2_ps, resROCPC2_sz, resROCPC2_cr, main = 'ROC Curves of PC2 predicting Status (AUC)', legends =  legends)
```

#### Psychosis Status

```{r, results='asis'}
ci(resROCPC2_ps)
```

#### Schizophrenia Status 

```{r, results='asis', }
ci(resROCPC2_sz)
```

#### Clinical Risk Status

```{r, results='asis'}
ci(resROCPC2_cr)
```


## 4. Predicting PS status based on adjusted PC2 

### Sex- and -age adjustment

Prediction strength for PS status drops slightly after adjustment. However, prediction of CR status becomes essentially random (see CI).

```{r, message=FALSE}
cov_df = data.frame(scores_df, 
                    penn_df[, c("case_id", "sex", "age", "race")]) %>%
  drop_na()
cov_df$PC2 = residuals(lm(PC2 ~ sex + age, data=cov_df))
resROCPC2_ps = roc(group ~ PC2, data=cov_df)

sz_cr_df = data.frame(bbl_id = penn_df$bbl_id,
                   group = penn_df$group,
                   age = penn_df$age) %>%
  drop_na()

sz_cr_df$PC2 = cov_df$PC2

sz_cr_df = merge(sz_cr, sz_cr_df %>% select(!group),
              by = 'bbl_id') 
sz_df = sz_cr_df %>%
  filter(group != 'CR')

resROCPC2_sz = roc(group ~ PC2, data=sz_df)

cr_df = sz_cr_df %>%
  filter(group != 'SZ')

resROCPC2_cr = roc(group ~ PC2, data=cr_df)
plotROCs(resROCPC2_ps, resROCPC2_sz, resROCPC2_cr, main = 'ROC Curves of PC2 predicting Status (AUC)',legends =  legends)
```

#### Psychosis Status

```{r, results='asis'}
ci(resROCPC2_ps)
```

#### Schizophrenia Status 

```{r, results='asis', }
ci(resROCPC2_sz)
```

#### Clinical Risk Status

```{r, results='asis'}
ci(resROCPC2_cr)
```

### Emotrics-adjusted scores

After adjusting PC2 for emotrics, prediction of SZ-status remains strong while CR becomes random.

```{r, message=FALSE}
emo_df$PC2 = residuals(lm(formula(sprintf("%s ~ %s", 
                                          "PC2", 
                                          paste(sig.emotrics, collapse=" + "))), 
                          data=emo_df))
resROCPC2_ps = roc(group ~ PC2, data=emo_df)

sz_df = merge(sz_cr, 
              emo_df[,c("bbl_id", "PC2")]) %>% 
  filter(group != 'CR')
resROCPC2_sz = roc(group ~ PC2, data=sz_df)

cr_df = merge(sz_cr, 
              emo_df[,c("bbl_id", "PC2")]) %>% 
  filter(group != 'SZ')
resROCPC2_cr = roc(group ~ PC2, data=cr_df)
plotROCs(resROCPC2_ps, resROCPC2_sz, resROCPC2_cr, main = 'ROC Curves of PC2 predicting Status (AUC)',legends =  legends)
```

#### Psychosis Status

```{r, results='asis'}
ci(resROCPC2_ps)
```

#### Schizophrenia Status 

```{r, results='asis', }
ci(resROCPC2_sz)
```

#### Clinical Risk Status

```{r, results='asis'}
ci(resROCPC2_cr)
```

### Height adjustment

```{r}
t(n_tbl["height",]) %>% kable(format="html") %>% kable_styling("striped", full_width = TRUE)
```

Prediction strength for PS status doesn't really change after adjusting for height. Adjusting for height increases prediction of SZ.

```{r, message=FALSE}
cov_height_df = data.frame(scores_df, 
                    penn_df[, c("case_id", "bbl_id", "height")]) %>%
  drop_na()
cov_height_df$PC2 = residuals(lm(PC2 ~ height, data=cov_height_df))
resROCPC2_ps = roc(group ~ PC2, data=cov_height_df)

sz_height_df = merge(cov_height_df %>%
                       select(!group),
                     sz_cr,
                     by="bbl_id") %>%
  filter(group != 'CR')

resROCPC2_sz = roc(group ~ PC2, data=sz_height_df)

cr_height_df = merge(cov_height_df %>%
                       select(!group),
                     sz_cr,
                     by="bbl_id") %>%
  filter(group != 'SZ')


resROCPC2_cr = roc(group ~ PC2, data=cr_height_df)

plotROCs(resROCPC2_ps, resROCPC2_sz, resROCPC2_cr, main = 'ROC Curves of PC2 predicting Status (AUC)',legends =  legends)
```

#### Psychosis Status

```{r, results='asis'}
ci(resROCPC2_ps)
```

#### Schizophrenia Status 

```{r, results='asis', }
ci(resROCPC2_sz)
```

#### Clinical Risk Status

```{r, results='asis'}
ci(resROCPC2_cr)
```

### Weight adjustment

```{r}
t(n_tbl["weight",]) %>% kable(format="html") %>% kable_styling("striped", full_width = TRUE)
```

Prediction strength for PS status drops slightly after adjusting for weight. Adjusting for weight decreases prediction of SZ.

```{r, message=FALSE}
cov_weight_df = data.frame(scores_df, 
                    penn_df[, c("case_id", "bbl_id", "weight")]) %>%
  drop_na()
cov_weight_df$PC2 = residuals(lm(PC2 ~ weight, data=cov_weight_df))
resROCPC2 = roc(group ~ PC2, data=cov_weight_df)

sz_weight_df = merge(cov_weight_df %>%
                       select(!group),
                     sz_cr,
                     by="bbl_id") %>%
  filter(group != 'CR')

resROCPC2 = roc(group ~ PC2, data=sz_weight_df)

cr_weight_df = merge(cov_weight_df %>%
                       select(!group),
                     sz_cr,
                     by="bbl_id") %>%
  filter(group != 'SZ')


resROCPC2 = roc(group ~ PC2, data=cr_weight_df)
plotROCs(resROCPC2_ps, resROCPC2_sz, resROCPC2_cr, main = 'PC2 predicting PS status',legends =  legends)
```

#### Psychosis Status

```{r, results='asis'}
ci(resROCPC2_ps)
```

#### Schizophrenia Status 

```{r, results='asis'}
ci(resROCPC2_sz)
```

#### Clinical Risk Status

```{r, results='asis'}
ci(resROCPC2_cr)
```

### GAF_C adjustment

```{r}
t(n_tbl["GAF_C",]) %>% kable(format="html") %>% kable_styling("striped", full_width = TRUE)
```

Prediction strength for PS status decreases after adjusting for GAF_C. Adjusting for GAF_C decreases prediction of SZ. Adjusting for GAF_C increases prediction of CR.

```{r, message=FALSE}
cov_gafc_df = data.frame(scores_df, 
                    penn_df[, c("case_id", "bbl_id", "GAF_C")]) %>%
  drop_na()
cov_gafc_df$PC2 = residuals(lm(PC2 ~ GAF_C, data=cov_gafc_df))
resROCPC2_ps = roc(group ~ PC2, data=cov_gafc_df)

sz_gafc_df = merge(cov_gafc_df %>%
                       select(!group),
                     sz_cr,
                     by="bbl_id") %>%
  filter(group != 'CR')

resROCPC2_sz = roc(group ~ PC2, data=sz_gafc_df)

cr_gafc_df = merge(cov_gafc_df %>%
                       select(!group),
                     sz_cr,
                     by="bbl_id") %>%
  filter(group != 'SZ')


resROCPC2_cr = roc(group ~ PC2, data=cr_gafc_df)
plotROCs(resROCPC2_ps, resROCPC2_sz, resROCPC2_cr, main = 'ROC Curves of PC2 predicting Status (AUC)',legends =  legends)
```

#### Psychosis Status

```{r, results='asis'}
ci(resROCPC2_ps)
```

#### Schizophrenia Status 

```{r, results='asis', }
ci(resROCPC2_sz)
```

#### Clinical Risk Status

```{r, results='asis'}
ci(resROCPC2_cr)
```

### GAF_H adjustment

```{r}
t(n_tbl["GAF_C",]) %>% kable(format="html") %>% kable_styling("striped", full_width = TRUE)
```

Prediction strength for PS status decreases after adjusting for GAF_H. Adjusting for GAF_H decreases prediction of SZ. Adjusting for GAF_H increases prediction of CR.

```{r, message=FALSE}
cov_gafh_df = data.frame(scores_df, 
                    penn_df[, c("case_id", "bbl_id", "GAF_H")]) %>%
  drop_na()
cov_gafh_df$PC2 = residuals(lm(PC2 ~ GAF_H, data=cov_gafh_df))
resROCPC2_ps = roc(group ~ PC2, data=cov_gafh_df)

sz_gafh_df = merge(cov_gafh_df %>%
                       select(!group),
                     sz_cr,
                     by="bbl_id") %>%
  filter(group != 'CR')

resROCPC2_sz = roc(group ~ PC2, data=sz_gafh_df)

cr_gafh_df = merge(cov_gafh_df %>%
                       select(!group),
                     sz_cr,
                     by="bbl_id") %>%
  filter(group != 'SZ')


resROCPC2_cr = roc(group ~ PC2, data=cr_gafh_df)
plotROCs(resROCPC2_ps, resROCPC2_sz, resROCPC2_cr, main = 'ROC Curves of PC2 predicting Status (AUC)',legends =  legends)
```

#### Psychosis Status

```{r, results='asis'}
ci(resROCPC2_ps)
```

#### Schizophrenia Status 

```{r, results='asis', }
ci(resROCPC2_sz)
```

#### Clinical Risk Status

```{r, results='asis'}
ci(resROCPC2_cr)
```

### MMSE adjustment

```{r}
t(n_tbl["MMSE",]) %>% kable(format="html") %>% kable_styling("striped", full_width = TRUE)
```

Prediction strength for PS status decreases after adjusting for MMSe. Adjusting for MMSE decreases prediction of SZ. Adjusting for MMSE increases prediction of CR.

```{r, message=FALSE}
cov_mmse_df = data.frame(scores_df, 
                    penn_df[, c("case_id", "bbl_id", "MMSE")]) %>%
  drop_na()
cov_mmse_df$PC2 = residuals(lm(PC2 ~ MMSE, data=cov_mmse_df))
resROCPC2_ps = roc(group ~ PC2, data=cov_mmse_df)

sz_mmse_df = merge(cov_mmse_df %>%
                       select(!group),
                     sz_cr,
                     by="bbl_id") %>%
  filter(group != 'CR')

resROCPC2_sz = roc(group ~ PC2, data=sz_mmse_df)

cr_mmse_df = merge(cov_mmse_df %>%
                       select(!group),
                     sz_cr,
                     by="bbl_id") %>%
  filter(group != 'SZ')


resROCPC2_cr = roc(group ~ PC2, data=cr_mmse_df)
plotROCs(resROCPC2_ps, resROCPC2_sz, resROCPC2_cr, main = 'ROC Curves of PC2 predicting Status (AUC)',legends =  legends)
```

#### Psychosis Status

```{r, results='asis'}
ci(resROCPC2_ps)
```

#### Schizophrenia Status 

```{r, results='asis', }
ci(resROCPC2_sz)
```

#### Clinical Risk Status

```{r, results='asis'}
ci(resROCPC2_cr)
```

## Conclusion

- PC2 alone is predictive of PS status with an AUC of `r round(pROC::auc(resROCPC2), 3)`!
- There are variance differences on PC2 between PS groups (SZ vs CR).
- After adjustments, Schizophrenia (SZ) status remains well-predicted while Clinical Risk (CR) status does not.
