...
```{r}
x22q_df = data.frame(case_id = chop_df$case_id, PC1 = resPCA$ind$coord[, 1], PC2 = resPCA$ind$coord[, 2])
...
x22q_df = merge(x22q_df, emotrics_df, by = 'case_id')
res = psych::corr.test(x22q_df[, c("PC1", "PC2")], 
                       x22q_df[, emotrics], 
                       adjust='fdr')
...
rownames(cor.mat) = c('PC1', 'p-val', 'PC2', 'p-val')
cor.mat %>% t()
...
res.t.test = emo_df %>% select(all_of(emotrics)) %>% lapply(function(x) t.test(x[df$group == 'PS'], x[df$group == 'NC'], var.equal=TRUE))
```

For univariate t-tests, the only difference between groups was in Philtrum metrics:  

- Brow Height (R): `r apa::apa(res.t.test[["Brow_Height_Right"]])`.  
- Brow Height (L): `r apa::apa(res.t.test[["Brow_Height_Left"]])`.  
- Marginal Reflex Distance 1 (R): `r apa::apa(res.t.test[["Marginal_Reflex_Distance_1_Right"]])`.  
- Marginal Reflex Distance 1 (L): `r apa::apa(res.t.test[["Marginal_Reflex_Distance_1_Left"]])`.  
...
```{r}
cov_df = data.frame(scores_df, penn_df[, c("sex", "age", "race")])
...
## 5. Predicting PS status based on adjusted PC2 
