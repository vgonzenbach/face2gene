...
## Plot DMS Dx
patho_df %>% ggplot(aes(x=PC1, y=PC2, color = DSM.Dx)) + geom_point() + 
  geom_point(data = patho_df %>% select(PC1, PC2, DSM.Dx) %>% group_by(DSM.Dx) %>%  summarize_all(mean), size = 4, shape = 17) + scale_color_manual(values = c("magenta", "gold")) + ggtitle("22q-like PC scores for 22q sample coded by DSM.Dx")
```
...
tmp_df %>% ggplot(aes(x=PC1, y=PC2, color = PS)) + geom_point() + 
  geom_point(data = tmp_df %>% select(PC1, PC2, PS) %>% group_by(PS) %>%  summarize_all(mean), size = 4, shape = 17) + scale_color_manual(values = c("magenta", "gold")) + ggtitle("22q-like PC scores for 22q sample coded by PS")
```
